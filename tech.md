...
4. 数据链路层：
   ...
   - **同一局域网内帧传输、MAC 寻址**，MAC 地址、局域网通信（在小区里每一段路如何走）
...
2. 数据链路层：在局域网中通过mac地址寻找网卡设备，包裹的数据包括：业务数据data、源mac地址和目标mac地址。
3. 网络层：由路由器主导，IP地址通过ARP对应MAC地址，解析IP协议，包裹的数据包括：业务数据data、源IP和目标IP。
4. 传输层：由操作系统主导，通过TCP/UDP协议进行握手，建立端口连接，包裹的数据包括：业务数据data、源端口和目标端口。
